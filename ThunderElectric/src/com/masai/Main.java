...
public class Main {
	...
	// admin functionality
	private static void adminFunctionality(Scanner sc, Map<Integer, Unit> units, Map<String, Consumer> consumers,
			List<Transaction> transactions) throws InvalidDetailsException, UnitException, TransactionException {
		// admin login

		adminLogin(sc);

		UnitService prodService = new UnitServicesImpl();
		ConsumerService cusService = new ConsumerServiceImpl();
		TransactionService trnsactionService = new TransactionServiceImpl();
		int choice = 0;
		try {
			do {
				System.out.println("Press 1 Login with Admin account");
				System.out.println("Press 2 Register new consumers");
				System.out.println("Press 3 View all consumers");
				System.out.println("Press 4 View the bill of the consumer");
				System.out.println("Press 5 View all the bills");
				System.out.println("Press 6 View all bills paid and pending");
				System.out.println("Press 7 Delete consumer");
				choice = sc.nextInt();

				switch (choice) {
				case 1:
					String added = adminLogIn(sc, units, unitService);
					System.out.println(added);
					break;
				case 2:

					adminRegesterNewConsumers(units, unitService);
					break;
				case 3:

					adminViewAllConsumers(sc, units, unitService);
					break;
				case 4:

					String upt = adminViewBillOfConsumer(sc, units, unitService);
					System.out.println(upt);
					break;
				case 5:
					adminViewAllBills(consumers, consumerService);

					break;
				case 6:
					adminViewAllBillPaidAndPending(transactions, trnsactionService);
					break;
				case 7:
					adminDeleteConsumer(id, name);
					break;
				default:
	...
	public static String adminAddProduct(Scanner sc, Map<Integer, Unit> products, UnitService unitService) {
